...
#                                                                             #
# This Script: Plot Figure E2 Model Specification Robustness                  #
#                                                                             #
# Maximilian Boeck, Munich                                                    #
#-----------------------------------------------------------------------------#
rm(list=ls())
...
setting_level    = paste0(prior,"-",intspec,"-",varspec,"-level")
setting_ht       = paste0(prior,"-",intspec,"-",varspec,"-ht")
setting_g7       = paste0(prior,"-",intspec,"-",varspec,"-g7")
spec_baseline    = paste0("plag=",plag,"_draws=",draws,"_burnin=",burnin)
spec_plag2       = paste0("plag=2_draws=",draws,"_burnin=",burnin)
spec_plag4       = paste0("plag=4_draws=",draws,"_burnin=",burnin)
...
# load robustness: plag=2
load(paste0(dirName,setting_baseline,"_irfgrid_",spec_plag2,".rda"))
irffmgrid_post_plag2    = irffmgrid_post / conv_fac
rm(irffmgrid_post)

# load robustness: plag=4
load(paste0(dirName,setting_baseline,"_irfgrid_",spec_plag4,".rda"))
irffmgrid_post_plag4    = irffmgrid_post / conv_fac
rm(irffmgrid_post)

# load robustness: ht
load(paste0(dirName,setting_ht,"_irfgrid_",spec_baseline,".rda"))
irffmgrid_post_ht       = irffmgrid_post / conv_fac
rm(irffmgrid_post)
...
# load robustness: level
load(paste0(dirName,setting_level,"_irfgrid_",spec_baseline,".rda"))
irffmgrid_post_level    = irffmgrid_post / conv_fac
rm(irffmgrid_post)
...
for(plot_horz in c(1,5)){
  pdf(paste0("./figureE2",ifelse(plot_horz==1,"a","b"),"_ModelSpec_hor=",plot_horz,".pdf"), height=6.5, width=11)
  ...
  for(dd in 1:d){
    ...
    for(nn in 2:n){
      ...
      ylim1 = range(irffmgrid_post_baseline[c(2,4,6),nn,,plot_horz,], 
                    irffmgrid_post_plag2[4,nn,,plot_horz,],
                    irffmgrid_post_plag4[4,nn,,plot_horz,],
                    irffmgrid_post_level[4,nn,,plot_horz,],
                    irffmgrid_post_ht[4,nn,,plot_horz,], 
                    irffmgrid_post_g7[4,nn,,plot_horz,], na.rm=TRUE)
      ...
      # polygon(c(1:grid_size,rev(1:grid_size)), c(irffmgrid_post_baseline[1,nn,dd,plot_horz,],rev(irffmgrid_post_baseline[7,nn,dd,plot_horz,])),
      ...
      # polygon(c(1:grid_size,rev(1:grid_size)), c(irffmgrid_post_baseline[3,nn,dd,plot_horz,],rev(irffmgrid_post_baseline[5,nn,dd,plot_horz,])), 
      #         col = "grey65", border=NA)
      lines(irffmgrid_post_baseline[4,nn,dd,plot_horz,], lty=1, lwd=2, col="black")
      lines(irffmgrid_post_plag2[4,nn,dd,plot_horz,], col="darkred",        lwd=2)
      points(x_axis_grid, irffmgrid_post_plag2[4,nn,dd,plot_horz,x_axis_grid], col=alpha("darkred",0.8),        pch=16, cex=2)
      lines(irffmgrid_post_plag4[4,nn,dd,plot_horz,], col="aquamarine4",    lwd=2)
      points(x_axis_grid, irffmgrid_post_plag4[4,nn,dd,plot_horz,x_axis_grid], col=alpha("aquamarine4",0.8),    pch=10, cex=2)
      lines(irffmgrid_post_ht[4,nn,dd,plot_horz,],    col="darkorange",     lwd=2)
      points(x_axis_grid, irffmgrid_post_ht[4,nn,dd,plot_horz,x_axis_grid],    col=alpha("darkorange",0.8),     pch=12, cex=2)
      lines(irffmgrid_post_level[4,nn,dd,plot_horz,],    col="cornflowerblue", lwd=2)
      points(x_axis_grid, irffmgrid_post_level[4,nn,dd,plot_horz,x_axis_grid],    col=alpha("cornflowerblue",0.8), pch=2,  cex=2)
      lines(irffmgrid_post_g7[4,nn,dd,plot_horz,], col="darkorchid3",    lwd=2)
      points(x_axis_grid, irffmgrid_post_g7[4,nn,dd,plot_horz,x_axis_grid], col=alpha("darkorchid3",0.8),    pch=4,  cex=2)
  ...
  plot(-10,-10,axes=FALSE,ylim=c(0,1),xlim=c(0,1))
  legend("center", c("Baseline","p=2","p=4","Gordon-Krenn","(Log-)Level","G7"), 
         lty=c(1,1,1,1,1,1), pch=c(NA,16,10,12,2,4), pt.cex=c(1,2,2,2,2,2),
         col=c("black","darkred","aquamarine4","darkorange","cornflowerblue","darkorchid3"), 
