...
#                                                                             #
# This Script: Plot Figure E6 LMIs kmeans                                     #
#                                                                             #
...
data_list   = lapply(data,function(l){
  tmp = as.matrix(l[,ivars])
  tmp = tmp[!apply(tmp,1,function(x)any(is.na(x))),]
})
cN_bigT     = unlist(lapply(data_list,nrow))
data_scaled = apply(do.call("rbind",data_list),2,scale)
set.seed(1)
kmeans_cl   = kmeans(data_scaled, centers=2, iter.max=10000)
if(kmeans_cl$centers[1,1] > kmeans_cl$centers[2,1]) gnames <- c("high","low") else gnames <- c("low","high")
cluster     = kmeans_cl$cluster

cumul_bigT  = c(0,cumsum(cN_bigT))
Dum_list    = list()
for(cc in 1:N){
  Dum_list[[cc]] = as.matrix(as.numeric(cluster[(cumul_bigT[cc]+1):cumul_bigT[cc+1]] == 2))
}
GINDEX <- matrix(NA_real_,N,2,dimnames=list(cN, gnames))
for(cc in 1:N){
  for(gg in 1:2){
    GINDEX[cc,gg] <- sum(Dum_list[[cc]] == (gg-1))
  }
}
CNINDEX <- matrix(NA_real_, N, 2, dimnames=list(cN, gnames))
for(cc in 1:N){
  for(gg in 1:2){
    CNINDEX[cc,gg] <- ifelse(GINDEX[cc,gg] > .5*sum(GINDEX[cc,]), 1, 0)
  }
}

COLINDEX_bg = ifelse(CNINDEX[,"low"]==1,"darkorange2","deepskyblue4")
COLINDEX_fr = ifelse(CNINDEX[,"low"]==1,"tan4","darkblue")


# plotting of LMIs
pdf(file="./figureE6_LMIs_Heterogeneity.pdf", height=4, width=12)
...
for(dd in 1:d){
  ...
  cols_bg     = COLINDEX_bg[data_order]
  cols_fr     = COLINDEX_fr[data_order]
  
  if(dd == 1) ylim1=c(0,100)
  if(dd == 2) ylim1=c(0,60)
  if(dd == 3) ylim1=c(0,7)
  ...
  if(dd == 1)
    title(expression(paste("Union density (UD, ",eta,")")), cex.main=2)
  if(dd == 2)
    title(expression(paste("U. benefit repl. rates (BRR, ",varphi,")")), cex.main=2)
  if(dd == 3)
    title(expression(paste("Employment protection (EPL, ",varsigma,")")), cex.main=2)
  ...
  for(cc in 1:N){
    ...
    points(x=points_x, y=points_y, col=adjustcolor(cols_bg[cc], alpha.f = 0.8), pch=16, cex=1.3)
  }
