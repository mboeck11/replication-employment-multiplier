% =========================================================================
% dmp_baseline_all_frictions.mod  --  ALL FRICTIONS COMBINED
%
% Boeck, Crespo Cuaresma, and Glocker (2026):
% "Labor Market Institutions, Fiscal Multipliers, and
%  Macroeconomic Volatility"
%
% NK-DMP model with all extensions active simultaneously:
%   (i)  nominal price rigidities (Calvo)
%   (ii) non-Ricardian (hand-to-mouth) households (xi > 0)
%   (iii) real wage rigidity (rhorw > 0)
% Used to assess the joint effect of multiple frictions on fiscal
% multipliers (Appendix B robustness).
%
...
w=rhorw*w(-1) + (1-rhorw)*( (1-eta)*(mrs+bu)/(1-tau)+eta*( mc*mpl+Lambda*( kappa*theta(1)-bs(1)*(1-rhob)*Ft(1) ) ));
