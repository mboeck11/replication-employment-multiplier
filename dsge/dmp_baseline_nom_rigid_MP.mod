% =========================================================================
% dmp_baseline_nom_rigid_MP.mod  --  MONETARY POLICY SHOCK MODEL
%
% Boeck, Crespo Cuaresma, and Glocker (2026):
% "Labor Market Institutions, Fiscal Multipliers, and
%  Macroeconomic Volatility"
%
% NK-DMP model with two exogenous shocks: government spending (vg) and
% monetary policy (vMP). Nominal price rigidities (Calvo). Representative
% Ricardian households only (no non-Ricardian block). Simplified Taylor
% rule. Used for auxiliary analyses of monetary transmission.
%
...
vMP          % monetary policy shock (to interest rate)
;  
...
pi=betta*pi(+1) + (1-thetaP)*(1-thetaP*betta)/thetaP * mc;
i=0.0*i(-1) + phi_pi * pi + vMP;
R = i-pi(+1);
...
% Wage bargaining
w=(1-eta)*(mrs+bu)/(1-tau)+eta*( mc*mpl+Lambda*( kappa*theta(1)-bs(1)*(1-rhob)*Ft(1) ) ); 

% Market clearing
y=c+g+kappa*v*0+Ft*(1-rhob)*(n(-1)+v(-1)*q(-1))*bs;
...
%% IRFs
stoch_simul(irf=20,order=1,nocorr,noprint);
%stoch_simul(irf=20,order=1,nofunctions,nograph,nocorr,noprint) % hp_filter=1600,pruning,
%ylog u clog; % nlog thetalog wlog glog S mrs mpl Hn Fn ulog; % yn ls mu pi i;
