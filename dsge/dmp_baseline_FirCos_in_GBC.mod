% =========================================================================
% dmp_baseline_FirCos_in_GBC.mod  --  FIRING COSTS AS GOVERNMENT REVENUE
%
% Boeck, Crespo Cuaresma, and Glocker (2026):
% "Labor Market Institutions, Fiscal Multipliers, and
%  Macroeconomic Volatility"
%
% DMP baseline model in which firing costs (b^s) are treated as revenues
% in the government budget constraint rather than as resource costs.
% Corresponds to the FCBC = 1 case in dmp_baseline_nom_rigid.mod.
% Used for Appendix B robustness (Figure B4): assessing whether the
% fiscal treatment of firing costs affects the multiplier.
%
...
Ft*(1-rhob)*(n(-1)+v(-1)*q(-1))*bs+tau*w*n+B=R(-1)*B(-1)+bu*u+Ts+g;
bu=varphi*w(-1);
bs=varsigma*w(-1);
...
bs=varsigma*w;
mrs=zeta*mpl;                
c=y-g-kappa*v-0*Ft*(1-rhob)*(n+v*q)*bs;               
...
Lambda=betta;
B=(Ft*(1-rhob)*(n+v*q)*bs+ tau*w*n-Ts-bu*(1-n)-g)/(R-1);
